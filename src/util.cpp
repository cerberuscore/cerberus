...
const char * const BITCOIN_CONF_FILENAME = "cerberus.conf";
const char * const BITCOIN_PID_FILENAME = "cerberusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cerberus" is a composite category enabling all Cerberus-related debug output
            if(ptrCategory->count(string("cerberus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CerberusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CerberusCore
    // Mac: ~/Library/Application Support/CerberusCore
    // Unix: ~/.cerberuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CerberusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CerberusCore";
#else
    // Unix
    return pathRet / ".cerberuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cerberus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cerberus.conf
