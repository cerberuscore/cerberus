...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Cerberus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Cerberus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Cerberus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Cerberus 0.7 datadir."
    ;;
  3)
    echo "Detected Cerberus pre-0.8 datadir."
    ;;
  4)
    echo "Detected Cerberus 0.8 datadir."
    ;;
esac
