 ...
 *
 *   // network
 *   CCerberusNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.cerberus.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CCerberusNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CCerberusNetwork(UniValue objIn)
    {
...
// // can be under: CerberusNetwork
// //   -- signature requirements : Key1(User)
// class CCerberusNetworkVariable : public CGovernanceObject
// {
// private:
